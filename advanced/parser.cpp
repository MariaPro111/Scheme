...
std::shared_ptr<Object> Read(Tokenizer* tokenizer) {
    ...
    } else if (QuoteToken* qt = std::get_if<QuoteToken>(&token)) {
        std::shared_ptr<Symbol> ptr = std::make_shared<Symbol>("'");
        tokenizer->Next();
        std::shared_ptr<Cell> new_node = std::make_shared<Cell>();
        new_node->first_ = std::make_shared<Symbol>("'");
        new_node->second_ = Read(tokenizer);
        return new_node;
    } else {
